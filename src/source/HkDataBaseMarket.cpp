...
bool HkLoadCommoditiesBaseMarket()
{
	...
	//Read commodity market items
	if(!ini.open("..\\data\\equipment\\market_commodities.ini", false))
		return false;
	...
	while(ini.read_header())
	{
		try {
			...
			biBase->lstMarketCommodities.clear();
			if(!ini.is_value("MarketGood"))
				throw "";
			...
			do {
				...
				mi.iArchID = CreateID(szEquipName);
				mi.fRep = ini.get_value_float(2);
				biBase->lstMarketCommodities.push_back(mi);
			} while(ini.read_value());
