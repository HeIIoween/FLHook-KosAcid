...
{
	wstring wscTRADataColor = L"FFFFFF"; // universe chatcolor
	...
	wstring wscXML = L"<TRA data=\"0x" + wscTRADataColor + wscTRADataFormat + L"\" mask=\"-1\"/><TEXT>" + XMLText(wscSender + L": " + wscMessage) + L"</TEXT>";

	return HkFMsgU(wscXML);
}

HK_ERROR HkMsgU(wstring wscSender, wstring wscMessage, wstring wscFont)
{
	wstring wscXML = L"<TRA font=\"" + wscFont + L"\" color=\"#FFFFFF\"/><TEXT>" + XMLText(wscSender + L": " + wscMessage) + L"</TEXT>";
...
HK_ERROR HkSetAFK(uint iClientID, wstring awayMsg) //an empty string means to unset away status
{
	if(!HkIsValidClientID(iClientID))
		return HKE_INVALID_CLIENT_ID;
	ClientInfo[iClientID].wscAfkMsg = awayMsg;
	return HKE_OK;
}
