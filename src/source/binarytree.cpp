...
/*NOTE: You cannot use fundamental data types!*/
...
BinaryTreeNode<T> *BinaryTree<T>::Add(T *pT,int iOwner)
{
    ...
    if (!pRoot)
        pRoot = new BinaryTreeNode<T>(pT,iOwner?true:false);
    else
...
int BinaryTree<T>::Delete(T *pT)
{
    ...
    if (*pRoot->pObj == *pT)  // Deleting root of tree
    {
        ...
        (pRoot->iRefCount)--;
        // Any references remaining?
        if (pRoot->iRefCount)
            return pRoot->iRefCount; // return the remaining ref count
...
T *BinaryTree<T>::FindPartial(T *pPartialT,int bGuaranteeUnique) const
{
    ...
    {
        const BinaryTreeNode<T> *pNode = pRoot->FindPartial(pPartialT,bGuaranteeUnique);
...
int BinaryTreeNode<T>::AddOffspring(T *pNew,int iOwner)
{
    ...
    if (*pNew < *pObj)
    {
        ...
        {
            pLeft = new BinaryTreeNode<T>(pNew,iOwner?true:false);
            return 1;
        }
    }
    else
    {
        ...
        {
            pRight = new BinaryTreeNode<T>(pNew,iOwner?true:false);
            return 1;
        }
    }
}
...
{
    if (pMyTree->pRoot == NULL || lIndex==-1)
        return NULL;
...
template <class T>
T *BinaryTreeIterator<T>::Find(T *key)
{
    ...
    while (!(*(pNodeList[iDepth]->pObj) == *key))
    {
        if (*(pNodeList[iDepth]->pObj) < *key)
            pNodeList[iDepth+1] = pNodeList[iDepth]->pRight;
        else
            pNodeList[iDepth+1] = pNodeList[iDepth]->pLeft;
        ...
        if (pNodeList[iDepth]==NULL) // not found and fell off the end of the tree
        {
            iDepth = -1;
            lIndex = -1;
            return NULL;
